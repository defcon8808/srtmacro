...
if (document.URL.substring(0, dsturl1.length) == dsturl1) {
	...
	$(document).ready(function() {
		chrome.storage.sync.get("excludeMergeTicket", function(options) {
			...
			if (sessionStorage.getItem('macro') == "true") {
				$("div#search_top_tag.tal_c.mgt30").append('<a href="#" onclick="macrostop();" id="btnstop" style="margin-left:5px;display:inline-block;height:100%;vertical-align:middle;"><img src="' + chrome.runtime.getURL('images/btn_stop.png') + '"></a>');			
			} else {
				$("div#search_top_tag.tal_c.mgt30").append('<a href="#" onclick="macrostart();" id="btnstart" style="margin-left:5px;display:inline-block;height:100%;vertical-align:middle;"><img src="' + chrome.runtime.getURL('images/btn_start.png') + '"></a>');	
			}	
						...
						if (coachSelected.indexOf(i+"") > -1) {
							...
							if (coachSpecials.length != 0) {
								for (j = 0; j < coachSpecials.length; j++) {
									...
									text = $(coachSpecials[j]).text();

									if (name == 'btn_small btn_burgundy_dark val_m wx90') {
										if (options.excludeMergeTicket) {										
											if (text == '예약하기') {
